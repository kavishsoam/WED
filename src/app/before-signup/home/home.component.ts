...
export class HomeComponent implements OnInit {
  ...
  aboutUs : any;
  videos : any = '';
  constructor(public httpService : HttpService,public commonService : CommonService,private sanitizer: DomSanitizer,public router : Router, public routes : ActivatedRoute) { 
    this.registratioForm1 = new FormGroup({        
      countryCode : new FormControl('+91'),
      username: new FormControl('', [Validators.required,Validators.pattern('[a-zA-Z ]*')]),
      mobile: new FormControl('', [Validators.required,Validators.pattern("^[0][1-9]\\d{9}$|^[1-9]\\d{6,15}$"), Validators.minLength(7)]),
      emailId: new FormControl('', [Validators.required, Validators.pattern(/^([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,3})$/)]), 
      profileFor: new FormControl('', [Validators.required,Validators.pattern('[a-zA-Z ]*')]),
  ...
  getStaticContents() {
    this.httpService.get('static/getStaticContent?Type=HOME',0).subscribe(res => {
      ...
      if(res['responseCode'] == 200) {
        ...
        this.videos = this.sanitizer.bypassSecurityTrustResourceUrl(url);
        // let url = "https://www.youtube.com/embed/-ReZ7DECScU";
        // this.videos = this.sanitizer.bypassSecurityTrustResourceUrl(url);
      }
    },err => {
      console.log('homeContent_Err ==>',err);
    })
  }
